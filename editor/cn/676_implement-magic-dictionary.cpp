...
public:
    ...
    bool toSearch(int index, string &s, MagicDictionary *t, bool flag) {

        if (index == s.size()) {
            return t->isEnd && flag;
        }

        if (t->next[s[index] - 'a'] != nullptr) {
            if (toSearch(index + 1, s, t->next[s[index] - 'a'], flag))
                return true;
            if (!flag) {
                for (int i = 0; i < 26; i++) {
                    if (i == s[index] - 'a')
                    ...
                    if (t->next[i] != nullptr) {
                        ...
                        if (toSearch(index + 1, s, t->next[i], true))
                            return true;
    ...
    bool search(string searchWord) {
        return toSearch(0, searchWord, this, 0);
    }
